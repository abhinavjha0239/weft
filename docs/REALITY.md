...
| Search (messages, v1) | **PROVEN (core)** | `GET /search`: Postgres FTS over a GENERATED search_tsv column (native + imported messages auto-indexed), ADR-010 operator subset (from:/in:/has:link/is:resolved/before:/after: + quoted phrases), ACL-scoped to member channels (S-2 slice), ts_rank ordering + plain-text highlighted snippets; parser unit-tested + `TestSearch` (FTS, operators, ACL isolation, empty-query 400). Client search box browser-verified. Gaps: full JQL-style work-item/thread search, was:/changed: temporal (event-log), pgvector semantic lane, public-non-member visibility, cross-entity (files/people/channels) |
| Notifications / files / work items / automations / compliance | DORMANT | schema + ADRs only |
| Importers (founding requirement) | **PROVEN (Zulip, first slice)** | `weftd import-zulip`: topics → titled Threads, ORIGINAL timestamps end-to-end (entities + event-log occurred_at/recorded_at split — E3 proven), provenance-idempotent re-runs (D5), email-match or claimable placeholders (D4), mention re-resolution, our-engine re-render, dry-run fidelity report with named losses + visible collision renames — `TestZulipImportShowcase`. Gaps: DMs/huddles, edit history, attachment/avatar/emoji files, user groups, per-user flags & sub prefs, tarball unpack, per-chunk streaming for multi-GB exports, Slack/Jira adapters |
